...
public record GetHostShowDetailResponse(Long showId, String title, String posterUrl, VenueInfo venue, ShowPeriod show,
    ReservationPeriod reservation, String description, Integer purchaseLimit, Integer likeCount, List<GradeInfo> grade,
    List<StakeholderInfo> stakeholders, List<RefundPolicyInfo> refundPolicy, List<SessionInfo> sessionInfo,
    ShowStatus status, LocalDateTime createdAt, LocalDateTime updatedAt) {
    ...
    public record StakeholderInfo(StakeholderRole role, Long userId, Integer shareBps) {
    }
