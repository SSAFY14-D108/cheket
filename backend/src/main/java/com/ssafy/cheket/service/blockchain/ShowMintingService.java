 ...
 *
 * [기획서 7.1 — 2단계: 예매 오픈 D-1]
 * 예매 시작 전날에 스케줄러가 자동 실행하여:
 *   ① EventNFT 발행 (공연 정보 + StakeholderNFT 참조 + 예매 규칙 온체인 확정)
 *   ② EventNFT.addSession() (회차별 등록)
 *   ③ EventNFT.setRefundPolicy() (환불 정책 온체인 확정)
 *   ④ TicketNFT.batchMintTickets() (100개씩 배치 발행, 플랫폼 지갑 소유)
 *
 * [왜 D-1에 발행하는가?]
 * - D-1까지는 공연 정보를 수정할 수 있음 (좌석 가격, 예매 기간 등)
 * - 온체인에 기록된 후에는 변경 불가 → 투명성 확보
 * - 예매 오픈 전에 전체 TicketNFT가 존재해야 totalSupply 검증 가능
 *
 * [상태 전이]
 * DRAFT → MINTING → MINTED
 *   DRAFT:   공연 정보 입력 완료, 아직 온체인 미등록
 *   MINTING: EventNFT + TicketNFT 발행 중 (중간 상태)
 *   MINTED:  발행 완료, 예매 가능 상태
 */
@Slf4j
@Service
...
public class ShowMintingService {
     ...
     * [실행 흐름]
     * ① DRAFT → MINTING 상태 전이
     * ② EventNFT.createEvent() — 공연 정보 온체인 등록, onChainEventId 추출 → DB 저장
     * ③ EventNFT.addSession() × N — 회차별 등록, onChainSessionId 추출 → DB 저장
     * ④ EventNFT.setRefundPolicy() — 환불 정책 온체인 등록
     * ⑤ TicketNFT.batchMintTickets() × M — 100개씩 배치 발행
     * ⑥ MINTING → MINTED 상태 전이
     *
     * @Transactional: 어느 단계에서든 실패하면 DB 전체 롤백
     * 단, 이미 온체인에 기록된 TX는 롤백 불가 (블록체인 특성)
     */
    @Transactional
     ...
     *
     * [온체인에 기록되는 것]
     * - stakeholderTokenIds: StakeholderNFT ID 배열 (수익 분배 참조)
     * - metadataCID: IPFS 메타데이터 CID (포스터/설명)
     * - totalSupply: 총 좌석 수 (모든 회차 합산)
     * - maxPerWallet: 1인당 최대 구매 수
     * - resaleCapBps: 리세일 가격 상한 (10000 = 원가 100%)
     * - bookingStartTime/EndTime: 예매 기간 (unix timestamp)
     *
     * [기획서 핵심]
     * "예매 오픈 전 전체 TicketNFT가 온체인에 발행된다.
     *  누구나 totalSupply를 조회하여 실제 발행 수량과 좌석별 가격을
     *  독립 검증할 수 있다 — 발행 수량 불투명 문제를 원천 차단"
     */
    private BigInteger mintEventNft(Show show) throws Exception {
        ...
        List<BigInteger> stakeholderTokenIds = stakeholders.stream()
            .map(s -> BigInteger.valueOf(s.getStakeholderNftId()))
        ...
        log.info("[공연 {}] EventNFT 발행 시작 — totalSupply={}, maxPerWallet={}, stakeholders={}",
            show.getId(), totalSupply, show.getPurchaseLimit(), stakeholderTokenIds.size());
        ...
        TransactionReceipt receipt = blockchainService.getEventNFT()
            .createEvent(
                ...
                BigInteger.valueOf(show.getPurchaseLimit()),
                BigInteger.valueOf(10000), // resaleCapBps: 원가 100%까지 리세일 허용
                bookingStart,
                bookingEnd
            ).send();

        // TransactionReceipt에서 EventCreated 이벤트 파싱 → eventId 추출
        List<EventNFT.EventCreatedEventResponse> events =
            blockchainService.getEventNFT().getEventCreatedEvents(receipt);
        ...
        log.info("[공연 {}] EventNFT 발행 완료 — onChainEventId={}, txHash={}",
            show.getId(), onChainEventId, receipt.getTransactionHash());
     ...
     *
     * [온체인에 기록되는 것]
     * - eventId: 이 회차가 속한 EventNFT ID
     * - sessionTimestamp: 공연 일시 (unix timestamp)
     * - ticketSupply: 이 회차의 좌석 수
     *
     * [다회차 공연 예시]
     * 10/5 공연 → Session #0 (250석)
     * 10/6 공연 → Session #1 (250석)
     * → 각각 독립적으로 정산 가능 (기획서 7.5)
     */
    private void addSessionOnChain(Session session, BigInteger onChainEventId) throws Exception {
        ...
        TransactionReceipt receipt = blockchainService.getEventNFT()
            .addSession(onChainEventId, sessionTimestamp, BigInteger.valueOf(ticketSupply))
            .send();

        // SessionAdded 이벤트에서 onChainSessionId 추출
        List<EventNFT.SessionAddedEventResponse> events =
            blockchainService.getEventNFT().getSessionAddedEvents(receipt);
     ...
     *
     * [온체인에 기록되는 것]
     * - daysArray: 남은 일수 배열 (내림차순: 7, 3, 1)
     * - rateBpsArray: 환불 비율 배열 (10000, 5000, 0)
     *
     * [기획서 핵심 — 왜 온체인?]
     * 백엔드가 환불액을 계산하면 → 조작 가능
     * 컨트랙트가 on-chain에서 직접 계산하면 → 조작 불가 (투명성)
     * Settlement.refund()가 이 정책을 읽어서 환불액 자동 계산
     */
    private void setRefundPolicyOnChain(Show show, BigInteger onChainEventId) throws Exception {
        List<RefundPolicy> policies =
            refundPolicyRepository.findByShowIdOrderByDaysRemainingDesc(show.getId());
        ...
        blockchainService.getEventNFT()
            .setRefundPolicy(onChainEventId, daysArray, rateBpsArray)
            .send();
     ...
     *
     * [기획서 7.2 — Batch Minting]
     * "5,000석 공연 — 예매 오픈 D-1 자동 실행
     *  → 100개씩 배치 분할
     *  → TX 큐에 순차 투입
     *  → TX 50개로 감소 (99% 감소)"
     *
     * [배치 발행 전략]
     * 같은 구역 + 같은 열 + 같은 등급의 연속 좌석을 한 TX로 묶음
     * batchMintTickets(플랫폼, eventId, sessionId, "A구역", 1열, 1번, 20개, "VIP", 50000)
     * → A구역 1열 1~20번 좌석이 1 TX로 발행
     *
     * [소유권]
     * 모든 TicketNFT는 플랫폼 지갑 소유로 발행
     * 구매 시 PurchaseRouter가 transferFrom(플랫폼 → 구매자)으로 이전
     * → mint보다 가스 적고 실패율 낮음
     */
    private void batchMintTicketsForSession(
        Show show, Session session, BigInteger onChainEventId
    ) throws Exception {
        ...
        List<Long> seatIds = sessionSeats.stream()
        ...
        List<Long> sectionIds = seatMap.values().stream()
            .map(Seat::getSectionId)
            .distinct()
            .collect(Collectors.toList());
        ...
        Map<Long, SeatGrade> gradeMap = seatGrades.stream()
            .collect(Collectors.toMap(SeatGrade::getSectionId, g -> g));
        ...
        //     B구역 1열 R석 30개     → 1 TX
        Map<String, List<SessionSeat>> groups = sessionSeats.stream()
        ...
        for (Map.Entry<String, List<SessionSeat>> entry : groups.entrySet()) {
            ...
            for (int i = 0; i < groupSeats.size(); i += BATCH_SIZE) {
                ...
                log.info("[공연 {}][회차 {}] 배치 발행: {}구역 {}열 {}번~{}번 ({}개)",
                    show.getId(), session.getId(),
                    section.getSectionName(), firstSeat.getRowNum(),
                    startSeatNum, startSeatNum + count - 1, count);

                // ===== 온체인 TX 전송 — batchMintTickets() =====
                TransactionReceipt receipt = blockchainService.getTicketNFT()
                    .batchMintTickets(
                        platformAddress,                             // to: 플랫폼 지갑 소유
                ...
                List<TicketNFT.BatchTicketMintedEventResponse> mintEvents =
                    blockchainService.getTicketNFT().getBatchTicketMintedEvents(receipt);
        ...
        log.info("[공연 {}][회차 {}] 배치 발행 완료 — 총 {}개 TicketNFT 발행",
            show.getId(), session.getId(), totalMinted);
    }

    /**
     * LocalDateTime → unix timestamp (BigInteger) 변환
     * 블록체인의 block.timestamp는 초 단위 unix timestamp
     */
