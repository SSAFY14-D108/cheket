...
public class QueueServiceImpl implements QueueService {
    ...
    private QueueEnterResponse handleExistingQueueToken(Long userId, Long showId, Long sessionId, String queueToken,
        Long nowEpochSec) {
        QueueTokenMeta queueTokenMeta = queueRepository.getQueueTokenMeta(queueToken);
        ...
        if (status == QueueStatus.ACTIVE) {
            ...
            if (admitExpiresAt == null || nowEpochSec > admitExpiresAt) {
                // ACTIVE 만료 처리
                queueRepository.updateQueueStatus(queueToken, QueueStatus.EXPIRED);
    ...
    private void promoteWaitingToActive(Long showId, Long sessionId, Long nowEpochSec) {
        Long activeCount = queueRepository.countActive(sessionId);
        ...
        for (long i = 0; i < available; i++) {
            String nextQueueToken = queueRepository.getFirstWaitingToken(sessionId);
            if (nextQueueToken == null)
                return;

            QueueTokenMeta queueTokenMeta = queueRepository.getQueueTokenMeta(nextQueueToken);
            if (queueTokenMeta == null)
                continue;
            ...
            queueRepository.addActive(sessionId, nextQueueToken);
            queueRepository.updateQueueStatus(nextQueueToken, QueueStatus.ACTIVE);
