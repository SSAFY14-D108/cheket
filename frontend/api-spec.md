...
> 백엔드 서버가 아직 없으므로 실제 연결은 하지 않았습니다.
...
- Auth: 불필요
- Response: 예매 오픈 예정 공연 목록 (reservationDate, status 포함)
...
- ⚠️ TX 상태는 `GET /wallets/transactions/{txId}`로 폴링 (1-2초 간격)
...
| `SignupScreen` | 1-4 회원가입, 1-5 SMS발송, 1-6 SMS확인 | ⭐⭐⭐ 높음 |
...
| `PasswordChangeScreen` | 1-8 비밀번호재설정 | ⭐⭐ 중간 |
...
- `GET /api/v1/hosts/shows/{venueId}` — 구역 목록 조회
- `GET /api/v1/hosts/shows/effect` — 티켓 효과 목록 조회
