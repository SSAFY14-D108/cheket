...
export function ShowDetailView({ showDetail }: ShowDetailViewProps) {
  ...
  const totalShareBps =
    PLATFORM_FEE_BPS +
    visibleStakeholders.reduce(
      (sum, stakeholder) => sum + stakeholder.shareBps,
      0,
    );
  const shareDistributionItems = [
    {
      key: "platform",
      label: "플랫폼",
      sublabel: `${PLATFORM_FEE_BPS.toLocaleString()}bps 고정`,
      percentage: (PLATFORM_FEE_BPS / 100).toFixed(1),
      className: "bg-slate-100/80",
    },
    ...visibleStakeholders.map((stakeholder, index) => ({
      key: `stakeholder-${stakeholder.id ?? index}`,
      label: stakeholder.name?.trim() || `회원 ${stakeholder.id}`,
      sublabel: "정산 대상",
      percentage: (stakeholder.shareBps / 100).toFixed(1),
      className: "bg-muted/60",
    })),
  ];
  const sectionCardClass =
    "gap-0 rounded-2xl border-border/70 bg-muted/20 shadow-none";
  const sectionHeaderClass = "border-b border-border/70 px-6 py-5";
  const sectionContentClass = "px-6 py-5";
    ...
    <main className="mx-auto max-w-6xl px-6 py-10">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 text-muted-foreground">
          ...
          <span className="text-sm font-medium">
            {showDetail.likeCount.toLocaleString()}
          </span>
        </div>
      </div>
      ...
      <div className="mt-8 flex flex-col gap-8 lg:flex-row">
        <div className="w-full shrink-0 lg:w-80">
          <Card className="gap-0 overflow-hidden rounded-2xl border-border/70 bg-muted/20 py-0 shadow-none">
        ...
        <div className="flex flex-1 flex-col gap-6">
          <Card className={sectionCardClass}>
            ...
            <CardContent
              className={`${sectionContentClass} flex flex-col gap-4`}
            >
          ...
          <Card className={sectionCardClass}>
            <CardHeader className={sectionHeaderClass}>
              <CardTitle className="text-base">예매 및 좌석 정보</CardTitle>
            </CardHeader>
            <CardContent
              className={`${sectionContentClass} flex flex-col gap-4`}
            >
                ...
                <div className="grid gap-2">
                  {showDetail.sessionInfo.map((session, index) => (
                    <div
                      ...
                      className="flex items-center justify-between gap-4 rounded-xl bg-background/90 px-4 py-3"
                    >
                      ...
                      <div className="shrink-0 rounded-full bg-muted px-3 py-1 text-xs font-medium text-foreground">
                        정원 {session.capacity.toLocaleString()}명
                      </div>
                    </div>
                  ))}
                </div>
            ...
            >
              <div className="flex flex-col gap-2">
                ...
                <p className="text-xs text-muted-foreground">
                  총 정산 비율은 {PLATFORM_TOTAL_BPS.toLocaleString()}bps이며,
                  플랫폼 수수료 {PLATFORM_FEE_BPS.toLocaleString()}bps가 고정
                  적용됩니다.
                </p>
                <div className="space-y-2">
                  {shareDistributionItems.map((item) => (
                    <div
                      key={item.key}
                      className={`flex items-center justify-between gap-4 rounded-lg px-4 py-3 ${item.className}`}
                    >
                      <div className="min-w-0">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="mt-1 flex flex-col gap-1">
                  {showDetail.refundPolicy.map((policy, index) => (
                    <p
                      key={index}
                      className="rounded-lg bg-muted/60 px-4 py-3 text-sm text-foreground"
                    >
                      - 공연 {policy.daysRemaining}일 전: {policy.refundRate}%
                      환불
                    </p>
                  ))}
                </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 rounded-xl border border-destructive bg-card py-3 text-sm font-semibold text-destructive transition-colors hover:bg-destructive hover:text-primary-foreground"
            >
              삭제하기
            </button>
          </div>
