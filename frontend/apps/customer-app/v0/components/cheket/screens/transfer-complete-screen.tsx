...
export function TransferCompleteScreen() {
  ...
  const txSeed = `${navParams.ticketId ?? ''}${navParams.recipientPhone ?? ''}`
  const txHash = `0x${txSeed.replace(/[^a-zA-Z0-9]/g, '').toLowerCase().padEnd(12, '0').slice(0, 7)}...${txSeed.replace(/[^a-zA-Z0-9]/g, '').toLowerCase().padEnd(8, '0').slice(-5)}`
