...
export function PaymentScreen() {
  const { navParams, navigate, goBack, user, addTicket, addTx, events } =
    useApp();
  const event = events.find((item) => item.id === navParams.eventId);
  const selectedEventDate =
    event?.dates?.find((item) => item.id === navParams.eventDateId) ?? null;
  ...
  const [selectedReason, setSelectedReason] =
    useState<PurchaseFailureReason>("SOLD_OUT");
  ...
  if (success) {
    return (
      <div className="h-full overflow-y-auto bg-background">
        <div className="flex min-h-full flex-col items-center gap-6 p-6 pt-10 pb-32 text-center">
              ...
              <div className="flex items-center gap-3 text-sm">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="font-medium text-foreground">
                    {selectedEventDate?.label ?? event.date}
                  </p>
                </div>
  ...
  return (
    <AppShell showBack onBack={goBack} title="결제 확인" showBottomNav={false}>
      <div className="flex flex-col gap-4 p-4">
        ...
        <div
          className={`rounded-xl border p-4 ${hasSufficientBalance ? "border-border bg-card" : "border-red-500/30 bg-red-500/10"}`}
        >
          ...
          {!hasSufficientBalance && (
            <div className="mt-3 flex items-center gap-2">
              ...
              <p className="text-xs text-red-400">
                잔액이 부족합니다.{" "}
                {(totalPrice - user.ctkBalance).toLocaleString()} CTK를 추가로
                충전해야 합니다.
              </p>
            </div>
          )}
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          <p className="mb-3 text-xs text-muted-foreground">
            결제를 진행하려면 아래 버튼으로 사전 승인 절차를 완료해야 합니다.
          </p>
          <button
        ...
        >
          {simulateFailure ? "실패 화면 테스트" : "결제 완료"}
        </button>

        <p className="text-center text-xs text-muted-foreground">
          결제가 완료되면 선택한 좌석 정보로 NFT 티켓이 발급되며, 내 티켓에서
          바로 확인할 수 있습니다.
        </p>
      </div>
    </AppShell>
